...
def pytest_sessionstart(session):
    empty_storage = subprocess.run(["bash", "scripts/check_empty.sh", BASE_DIR], capture_output=True, text=True)

    if empty_storage.returncode == 0:
        print("💬 Initial cleaning of tempory files and empty storage before the tests...")
        subprocess.run(["bash", "scripts/clean.sh", "tempory_and_storage"], check=True)
    elif empty_storage.returncode == 1:
        print("💬 Initial cleaning of tempory files before the tests...")
        subprocess.run(["bash", "scripts/clean.sh", "tempory"], check=True)
