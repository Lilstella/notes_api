...
def test_render_tsv_to_html():
    file_name = "test_render_tsv_to_html"
    content = {"text": "name\tage\tcountry\nLouis\t43\tVen\nAlbert\t80\tArg", "filetype": "tsv"}

    client.post(f"/file/create/{file_name}", json=content)
    response = client.request(method="GET", url=f"/render/html/{file_name}", params={"file_type": "tsv"})
...
def test_render_html_to_html():
    file_name = "test_render_html_to_html"
    content = {"text": "<h1>Title</h1><p>This is a <strong>bold</strong> paragraph.</p>", "filetype": "html"}

    client.post(f"/file/create/{file_name}", json=content)
    response = client.request(method="GET", url=f"/render/html/{file_name}", params={"file_type": "html"})

    assert response.status_code == 200
    html = response.json()["content"]

    assert "<h1>Title</h1>" in html
    assert "<p>This is a <strong>bold</strong> paragraph.</p>" in html
